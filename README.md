### G'day, I'm Jack
- Co-Owner and Lead Developer at [Discord Mafia](https://discord.gg/social-deduction)
- Technical Administrator at [MafiaScum](https://www.mafiascum.net)

## Currently Working On
- Mafia Engine: Rust Rewrite

## Focused Projects
- [Mafia Engine](https://github.com/JacksonVirgo/mafia-engine-2.0) - The dedicated discord bot for Discord Mafia; running, automating and helping all users play and host games of mafia (and more)
- [MafiaScum Modtools - Browser Ext.](https://github.com/JacksonVirgo/mafiascum-modtools) - A browser extension to add useful functionality to the MafiaScum forums!
- [Discord/Github Issue Bridge](https://github.com/JacksonVirgo/github-discord-bridge) - A discord bot (written in Go) focused on bridging a discord forum channel with github issues to allow seamless communication across the two platforms

## Contact
You can catch me on Discord at `._melancholic_.`
